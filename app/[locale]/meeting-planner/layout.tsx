...
export async function generateMetadata({
  params,
}: LayoutProps<"/[locale]/meeting-planner">): Promise<Metadata> {
  const { locale } = await params;
  return toolMetadata("meeting-planner", locale);
}

export default async function Layout({
  children,
  params,
}: LayoutProps<"/[locale]/meeting-planner">) {
  const { locale } = await params;
  return (
    <>
      <JsonLd data={await toolJsonLd("meeting-planner", locale)} />
      {children}
    </>
  );
}
