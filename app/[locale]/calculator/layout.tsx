...
export async function generateMetadata({
  params,
}: LayoutProps<"/[locale]/calculator">): Promise<Metadata> {
  const { locale } = await params;
  return toolMetadata("calculator", locale);
}

export default async function Layout({
  children,
  params,
}: LayoutProps<"/[locale]/calculator">) {
  const { locale } = await params;
  return (
    <>
      <JsonLd data={await toolJsonLd("calculator", locale)} />
      {children}
    </>
  );
}
