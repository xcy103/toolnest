...
export async function generateMetadata({
  params,
}: LayoutProps<"/[locale]/gradient">): Promise<Metadata> {
  const { locale } = await params;
  return toolMetadata("gradient", locale);
}

export default async function Layout({
  children,
  params,
}: LayoutProps<"/[locale]/gradient">) {
  const { locale } = await params;
  return (
    <>
      <JsonLd data={await toolJsonLd("gradient", locale)} />
      {children}
    </>
  );
}
