...
export async function generateMetadata({
  params,
}: LayoutProps<"/[locale]/word-counter">): Promise<Metadata> {
  const { locale } = await params;
  return toolMetadata("word-counter", locale);
}

export default async function Layout({
  children,
  params,
}: LayoutProps<"/[locale]/word-counter">) {
  const { locale } = await params;
  return (
    <>
      <JsonLd data={await toolJsonLd("word-counter", locale)} />
      {children}
    </>
  );
}
